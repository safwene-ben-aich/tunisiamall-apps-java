...
// D�claration des m�thodes
  	...
  	/** M�thode permettant d'introduire le num�ro de port du serveur */
	...
	/** M�thode permettant de retourner le num�ro de port du serveur */
 	public int getPort(){
    	...
    	/** M�thode permettant de lancer le serveur*/
	private void launch(){
		logins[0]="Tout";
		clients.add("Tout");
		nbClients++;
		// 1. Cr�er la socket du serveur : ServerSocket(int _port)
		try{
      	...
      	System.out.println("Serveur en execution...");
			...
			while(true){
				...
				// 2. Appeler la m�thode 'accept()' du ServerSocket cr��e en 1.
				_s=ss.accept();
				// 3. Ajouter la nouvelle connexion dans un vecteur (ou tableau) contenant toutes les connexions
				clients.add(clients.size(),_s);
                nbClients++;

                // 4. Appeler la m�thode _t.start() permettant de lancer run() (run() : permet de traiter les connexions des clients)
				_t = new Thread(this);
				_t.start();
			}
        	}catch(Exception e){
	...
	public void run(){
		try{
			// 1. Retirer la derni�re socket du vecteur
			Socket sck=(Socket)clients.lastElement();
			int n=nbClients;
            while(true){
            	System.out.println("Debut reception");
            	// 2. R�cup�rer le flux d'entr�e de la socket (m�thode getInputStream() de la classe Socket)
...
//
// Le programme prendra 1 parametre : le numero de port � surveiller
//
//////////////////////////////////////////////////////////////////////////////////////////////
